...
case class Input(numbers: List[Int], boards: List[Board]):
  def part1: Int =
    @tailrec 
    def part10(n: List[Int], boards: List[Board]): Int =
      val v = n.head
      val mboards = boards.map(_.mark(v))
      mboards.find(_.isCompleted) match
        case Some(board) => board.score * v
        case _ => part10(n.tail, mboards)

    part10(numbers, boards)

  def part2: Int = 
    @tailrec
    def part20(n: List[Int], boards: List[Board], acc: List[Int]): List[Int] =
      n match
        case Nil => acc
        case v :: tail =>
          val mboards = boards.map(_.mark(v))
          mboards.find(_.isCompleted) match
            case Some(board) => part20(tail, mboards.filterNot(_.isCompleted), board.score * v :: acc)
            case _           => part20(tail, mboards, acc)

    part20(numbers, boards, Nil).head
